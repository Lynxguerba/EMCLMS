...
import { useAdminFacultyLogs, useAdminAllUsers } from "../hooks/useQueries";
...
const getInitials = (firstName?: string, lastName?: string): string => {
 if (!firstName && !lastName) {
  return "F"; // Default to 'F' for Faculty
 }
...
export default function AdminFacultyLogs() {
 ...
 const { data: rows = [], isLoading: logsLoading } = useAdminFacultyLogs();
 ...
 // Helper function to get user details by faculty name or ID
 const getUserByFaculty = useCallback((facultyName: string, facultyId?: string): User | null => {
  if (facultyId) {
   const userById = users.find(u => u.user_id === facultyId);
 ...
 const filteredRows = useMemo(() => {
  return rows.filter((log: LogEntry) => {
   ...
   const userMatch = getUserByFaculty(log.instructor, log.instructor_id);
   return (
 ...
 }, [searchQuery, rows, getUserByFaculty]);
 ...
 // Render layout with navbar, sidebar, and faculty logs table
 return (
   ...
   <div className="min-h-screen pt-8 p-4 mt-6 md:-mt-2 md:ml-52">
    ...
    <div className="mb-6 grid items-center gap-2">
     ...
     <p className="text-1xl">
      View and monitor all faculty activity logs in the system.
     </p>
    </div>
    ...
    <div className="mb-5">
     <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
      <div className="relative flex-1 max-w-md">
       ...
       <input
        type="text"
        placeholder="Search by faculty name or message..."
        value={searchQuery}
      ...
      <table className="w-full">
       ...
       <tbody className="divide-y divide-gray-200">
        {loading ? (
         ...
         currentLogs.map((log) => {
          const userMatch = getUserByFaculty(log.instructor, log.instructor_id);
          const borderColorClass = getBorderColor(userMatch?.user_type || "Instructor");
